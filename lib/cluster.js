...
class Cluster extends AbstractPool {
  ...
  acquire(options) {
    ...
    const stats = this.stats(attributes);

    if (stats.available) {
      return this._acquireWhere(attributes);
    }

    return this._queueAcquire(options, () => {
      return this._waitForAvailableResource(attributes);
    });
  }
  ...
  _waitForAvailableResource(attributes) {
    ...
    // if only 1 pool is available, no point in racing..
    if (pools.length === 1) {
      return pools[0].acquire();
    }
